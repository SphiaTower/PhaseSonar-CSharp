...
namespace PhaseSonar.Analyzers.WithoutReference.LockOn {
    /// <summary>
    /// lock on the min point
    /// </summary>
    public class LockDipAccumulator : IAccumulator, ILockOnSeeker {
        ...
        public LockDipAccumulator([NotNull] ICrestFinder finder, ISlicer slicer, IPulsePreprocessor preprocessor, ICorrectorV2 corrector, double lockFreq, double lockScanFreqRadius, double sampleRateInMHz)  {
        ...
        public int SeekLockOnPoint(Complex[] spec, int start, int stop) {
            return Functions.FindMinIndex(spec, start, stop);
        }
    }
}